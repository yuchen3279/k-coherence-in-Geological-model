...
public:
    ...
    float& p(int i, int j) { return _buf[i+j*length]; }//_buf第一个是95个格子，的坐标存在了buf里面，是一个数组 
    float& p(int idx) { return _buf[idx]; }   
	int wrapl(int i) { return (i+length)%length; }
    int wrapw(int j) { return (j+width)%width; }

    unsigned ij_to_idx(int i, int j) 
           { return i+(j-1)*length; }
    void idx_to_ij(int idx, int &i, int &j)
           { i = idx%width; j = idx/width; }
